			...
			if(data.response != null){
				for(i=0; i< data.response.routes.length; i++){
					...
					for(j=0; j< data.response.routes[i].segments.length; j++){
							routes = routes + '<div class="col-md-4 col-xs-12">' +
													'<h4>Segment ' + (j + 1) + '</h4>';
					...
					var content = '<div class="col-md-8 col-md-push-2 col-md-pull-2 col-xs-12 route-data">' +
					...
					if(data.response.routes[i].indicativePrice.price == undefined){
						content = content + 		'<li>Estimated Price: Free </li>';
					}
					else
						content = content + 		'<li>Estimated Price: ' + data.response.routes[i].indicativePrice.price + ' ' + data.response.routes[i].indicativePrice.currency + '</li>';
				...
				$('.routes').append('<div class="col-md-8 col-md-push-2 col-md-pull-2 col-xs-12 route-data">' +
